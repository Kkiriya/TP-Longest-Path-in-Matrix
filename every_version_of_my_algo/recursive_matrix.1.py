...
def navigate(directions=['right','down','left','up'], position=(0,0), path_length=0):
    ...
    try: # contains the current value of the position within the matrix and the next one
        ...
        return navigate(directions[1:], position, path_length) # if we end up outside the matrix we move on to the next direction without adding to the path_length or changing the current position
    if next_position[0] < 0 or next_position[1] < 0:
        ...
        return navigate(directions[1:], position, path_length) # negative value work in python however they do not fit the logic of our code
    ...
    if current_val < next_val:
        ...
        return navigate(directions[1:], position, path_length) # if the next position is bigger then the current one we move on to the next direction without adding to the path_length or changing the current position
        ...
        return path_length + navigate(directions, next_position, path_length)
